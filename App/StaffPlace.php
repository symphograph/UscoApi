...
class StaffPlace
{
    ...
    private function initAva(): string
    {
        ...
        if (!file_exists(ServerEnv::DOCUMENT_ROOT() . '/' . $ava)) {
    ...
    public static function getPlaceIdByChair(int $groupId, int $chair): false|int
    {
        $chair += 1;
        $qwe = qwe2("
            SELECT * FROM places 
    ...
    public static function getUngrouped(string $completed, array $groups, string $date = ''): array
    {
        ...
        $qwe = qwe2("SELECT 
            ...
            FROM personal 
                ...
                inner join jobs j on e.jobId = j.id
                    and j.groupId = 10
            WHERE personal.id not in ($completed)", ['date' => $date]
        );
        if (!$qwe or !$qwe->rowCount()) {
            return $groups;
        }
    ...
    public function updatePlace(string $start = '', string $stop = ''): void
    {
        if (empty($start)) {
            $start = date('Y-m-d');
        }
    ...
    private static function dateFixer(int $persId, string $start): bool
    {
        $qwe = qwe2("
    ...
    public function setUngrouped($date = ''): void
    {
        if (empty($date)) {
