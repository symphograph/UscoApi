...
class Poster extends Img
{
    ...
    private function putToOriginals(array $file): bool
    {
        ...
        return @move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . $this->srcOrig);
    }
    ...
    {
        $image = new Imagick($_SERVER['DOCUMENT_ROOT'] . '/' . $this->srcOrig);
        ...
        if (!$image->writeImage($_SERVER['DOCUMENT_ROOT'] . $newFile)) {
            return false;
        }
